...
$publisher->appendChild($doc->createTextNode($feed_settings['indeed']['publisher'])); // put here the name of the publisher or ID
$r->appendChild($publisher);

$publisherurl = $doc->createElement("publisherurl");
$publisherurl->appendChild($doc->createTextNode($feed_settings['indeed']['publisherurl']));//Put here the url of your site
...
do {
    ...
    if (is_array($feed_jobs->posts) and !empty($feed_jobs->posts)) {
        ...
        foreach ($feed_jobs->posts as $the_post) {
            ...
            $description->appendChild($doc->createCDATASection(htmlspecialchars($the_post->post_content)));
